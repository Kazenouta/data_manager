...
TreeGrid = function(_config){
	...
	TreeGrid.json2str = function(obj){
		...
		var currTrData = eval("(" + '{' + arr.join(', ') + '}' + ")");
		if(currTrData.checked){
			return '{' + arr.join(', ') + '}';
		}else{
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			for(var j=0;j<_cols.length;j++){
				...
				//单元格内容
				if(col.handler){
					s += "<span class='getData'>"+(eval(col.handler + ".call(new Object(), row, col, rownum)") || "") + "</span></td>";
				}else{
					s += "<span class='getData'>"+(row[col.dataField] || "") + "</span></td>";
				}
			}
			s += "</tr>";
	...
	init = function(){
		...
		//将单击事件绑定到tr标签
		__root.find("tr span.getData").bind("click", function(){
			__root.find("tr").removeClass("row_active");
			jQuery(this).parents("tr").addClass("row_active");
			
			//获取当前行的数据
			__selectedData = jQuery(this).parents("tr")[0].data || jQuery(this).parents("tr")[0].getAttribute("data");
			__selectedId = jQuery(this).parents("tr")[0].id || jQuery(this).parents("tr")[0].getAttribute("id");
			__selectedIndex = jQuery(this).parents("tr")[0].rownum || jQuery(this).parents("tr")[0].getAttribute("rowIndex");

			//行记录单击后触发的事件
			if(_config.spanClick){
				eval(_config.spanClick + "(__selectedId, __selectedIndex, TreeGrid.str2json(__selectedData))");
			}
		});
		//展开、关闭下级节点
		__root.find("img[folder='Y']").bind("click", function(){
			var trid = this.trid || this.getAttribute("trid");
			var isOpen = __root.find("#" + trid)[0].getAttribute("open");
			if(isOpen == "Y"){
				isOpen = "N";
			}else{
				isOpen = "Y";
			}
			__root.find("#" + trid)[0].setAttribute("open", isOpen);
